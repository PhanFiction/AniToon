...
export default function Card({ showType, backgroundImg, title }){
  return (
    <article className="group w-28 md:w-40">
      <div className="flex flex-col relative h-full mb-2 group bg-darkBluePurple text-lime-300 hover:-translate-y-1 duration-500"
      >
        <div className="w-full h-full absolute left-0 top-0 group-hover:bg-[#8988885c] group-hover:backdrop-blur-[6px] duration-500">
          <p className="font-bold absolute bottom-2 left-2 opacity-0 duration-300 group-hover:opacity-100">{showType}</p>
          <svg
            className="h-[1.5em] w-[1.5em] absolute top-28 left-[68px] duration-300 opacity-0 group-hover:opacity-100"
            viewBox="0 0 48 48"
            fill="none"
            height="36"
            width="36"
        ...
        <img src={backgroundImg} alt={backgroundImg} className="w-full h-full object-cover" />
      </div>
      <p className="group-hover:text-red text-xs md:text-base font-medium duration-500 w-28 md:w-40 truncate">{title}</p>
    </article>
  );
};